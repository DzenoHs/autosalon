...
function BuyingProcessStrip() {
  // const navigate = useNavigate();
  const [isVisible, setIsVisible] = useState(false)

  // ✅ Memoized visibility trigger
  ...
  useEffect(() => {
    const clearTimer = triggerVisibility()
    return clearTimer
  }, [triggerVisibility])

  // ✅ Memoized steps array
  const steps = useMemo(
    () => [
      {
  ...
  return (
    <section
      id="buycar"
      className="relative py-12 sm:py-16 md:py-20 lg:py-28 bg-gradient-to-b from-black via-gray-900 to-black overflow-hidden"
    >
      {/* Background Effects */}
      <div className="absolute inset-0 opacity-5">
        <div className="absolute top-0 left-1/4 w-96 h-96 bg-cyan-500 rounded-full blur-xl"></div>
        <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-purple-500 rounded-full blur-xl"></div>
      </div>

      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div
          className={`mb-12 md:mb-20 text-center transform transition-all duration-500 will-change-transform ${
          ...
          <div className="relative flex items-center justify-between mb-8">
            ...
            {steps.map((step, idx) => (
              <div
                ...
                className={`relative flex flex-col items-center flex-1 z-10 transform transition-all duration-500 will-change-transform ${
                  isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
                }`}
              >
                ...
                <div className="relative group cursor-pointer">
                  <div
                    className={`w-16 h-16 lg:w-20 lg:h-20 flex items-center justify-center rounded-full bg-gradient-to-br ${step.gradient} ${step.shadow} shadow-2xl border-2 border-gray-800 group-hover:scale-110 transition-all duration-300 backdrop-blur-sm`}
                  >
                    <span className="text-white drop-shadow-lg">{step.icon}</span>
                  </div>
                  <div
                    className={`absolute inset-0 rounded-full bg-gradient-to-br ${step.gradient} opacity-0 group-hover:opacity-20 blur-xl transition-all duration-300`}
                  ></div>
                </div>
        ...
        <div className="md:hidden space-y-6">
          {steps.map((step, idx) => (
            <div
              ...
              className={`transform transition-all duration-500 will-change-transform ${
                isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
              }`}
            >
              <div className="relative backdrop-blur-xl bg-gradient-to-br from-gray-900/80 to-gray-800/60 border border-gray-700/50 rounded-2xl p-6 shadow-2xl hover:shadow-3xl transition-all duration-300">
                ...
                <div className="flex items-start space-x-4">
                  <div
                    className={`w-14 h-14 flex items-center justify-center rounded-xl bg-gradient-to-br ${step.gradient} ${step.shadow} shadow-lg flex-shrink-0`}
                  >
        ...
        {/* CTA Button */}
        <div
          className={`text-center mt-12 md:mt-16 transform transition-all duration-500 delay-100 will-change-transform ${
            isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
          }`}
        >
          <button
...
export default React.memo(BuyingProcessStrip)

...
export const getCarsGearbox = async (req, res) => {
  const startTime = Date.now();
  res.header('Access-Control-Allow-Origin', '*');
  res.header('Content-Type', 'application/json');
  ...
  try {
    ...
    const cacheKey = generateCacheKey({ endpoint: 'getCarsGearbox' });
    const cachedResult = getCachedData(cacheKey);
    ...
    const { allCars } = await fetchCarsFromMobileAPI(apiParams, 5);
    const distinctGearbox = [...new Set(allCars.map(ad => ad.gearbox).filter(Boolean))];

    setCachedData(cacheKey, distinctGearbox);

    return res.status(200).json({
      cached: false,
      data: distinctGearbox,
      responseTime: Date.now() - startTime,
      timestamp: new Date().toISOString(),
    });
  } catch (error) {
    console.error('❌ Error in getCarsGearbox API:', error);
    res.status(500).json({
      success: false,
      error: 'Internal server error',
      message: error.message,
...
export const getCarsFuel = async (req, res) => {
  const startTime = Date.now();
  res.header('Access-Control-Allow-Origin', '*');
  res.header('Content-Type', 'application/json');
  ...
  try {
    ...
    const cacheKey = generateCacheKey({ endpoint: 'getCarsFuel' });
    const cachedResult = getCachedData(cacheKey);
    ...
    const { allCars } = await fetchCarsFromMobileAPI(apiParams, 5);
    const distinctFuel = [...new Set(allCars.map(ad => ad.fuel).filter(Boolean))];

    setCachedData(cacheKey, distinctFuel);
    ...
    console.error('❌ Error in getCarsFuel API:', error);
    res.status(500).json({
      success: false,
      error: 'Internal server error',
      message: error.message,
